...
import { 
  ArrowRight, Play, Sparkles, Clock, Globe, Zap, Star, Trophy, Target, 
  Users, BookOpen, Brain, Lightbulb, Rocket, Shield, Heart, Award,
  TrendingUp, Download, MessageCircle, Video, Headphones, Eye
} from 'lucide-react';
import { Enhanced3DScene } from './Enhanced3DScene';
import { AdvancedHeroFeatures } from './AdvancedHeroFeatures';
import { InteractiveHeroStats } from './InteractiveHeroStats';

export const HeroSection: React.FC = () => {
  const [currentTime, setCurrentTime] = useState(new Date());
  const [activeFeature, setActiveFeature] = useState(0);
  const [onlineUsers] = useState(25847);
  const [todayDownloads] = useState(12453);
  const [studyHours] = useState(1847329);
  ...
  // Animated counters
  const onlineSpring = useSpring({
    from: { number: 0 },
    to: { number: isInView ? onlineUsers : 0 },
    config: { duration: 2000 }
  });

  const downloadsSpring = useSpring({
    from: { number: 0 },
    to: { number: isInView ? todayDownloads : 0 },
    config: { duration: 2500 }
  });

  const hoursSpring = useSpring({
    from: { number: 0 },
    to: { number: isInView ? studyHours : 0 },
  ...
  return (
    <motion.section 
      ref={heroRef}
      id="home" 
      className="relative min-h-[200vh] flex flex-col overflow-hidden"
      style={{ opacity }}
    >
      ...
      <motion.div
        ...
        style={{ y: y1, scale }}
      >
        <Enhanced3DScene className="absolute inset-0 opacity-80" />
      </motion.div>
      ...
      <motion.div 
        ...
        style={{
          background: `
            ...
            radial-gradient(circle at 60% 80%, rgba(16, 185, 129, 0.1) 0%, transparent 60%)
          `
        }}
        animate={{
          background: [
            ...
            `radial-gradient(circle at 80% 20%, rgba(139, 92, 246, 0.15) 0%, transparent 60%),
             ...
             radial-gradient(circle at 60% 60%, rgba(59, 130, 246, 0.1) 0%, transparent 60%),
             radial-gradient(circle at 40% 20%, rgba(16, 185, 129, 0.1) 0%, transparent 60%)`
          ]
        }}
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        >
          ...
          >
            <motion.div
              className="flex items-center gap-3 px-6 py-3 rounded-full glass backdrop-blur-xl border border-white/20"
            ...
            >
              ...
              <span className="text-sm font-semibold text-foreground">
                <animated.span>{onlineSpring.number.to(n => Math.floor(n).toLocaleString())}</animated.span> students online
              </span>
            </motion.div>
            ...
            >
              ...
              <span className="text-sm font-semibold text-foreground">
                <animated.span>{downloadsSpring.number.to(n => Math.floor(n).toLocaleString())}</animated.span> downloads today
              </span>
            </motion.div>
          </motion.div>
